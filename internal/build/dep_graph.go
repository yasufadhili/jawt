...
// dependencyGraph concrete implementation
type dependencyGraph struct {
	nodes        map[string]*GraphNode // All nodes in the graph
...
func (dg *dependencyGraph) GetCompilationOrder() ([]string, error) {
	// For compilation, we want dependencies compiled first
...
// Additional utility methods for build system integration
...
func (dg *dependencyGraph) collectTransitiveDeps(node string, visited map[string]bool, result *[]string) {
	...
	for _, dep := range dg.edges[node] {
		// TODO: Make sure we don't duplicate dependencies
