...
function generateGoTest(test: GoTest): string {
    ...
    const imports = [`"github.com/yasufadhili/jawt/internal/tsc/fourslash"`];
    // Only include these imports if the commands use them to avoid unused import errors.
    if (commands.includes("ls.")) {
        imports.push(`"github.com/yasufadhili/jawt/internal/tsc/ls"`);
    }
    if (commands.includes("lsproto.")) {
        imports.push(`"github.com/yasufadhili/jawt/internal/tsc/lsp/lsproto"`);
    }
    imports.push(`"github.com/yasufadhili/jawt/internal/tsc/testutil"`);
