...
// ProcessManager manages all external processes
...
func NewProcessManager(ctx context.Context, logger core.Logger, eventBus core.EventBus) *ProcessManager {
...
func (pm *ProcessManager) StartProcess(name string, options ProcessOptions) error {
	pm.mu.Lock()
	defer pm.mu.Unlock()
	...
	process := NewManagedProcess(name, options, pm.ctx, pm.logger, pm.eventBus)
...
func (pm *ProcessManager) GetProcesses() map[string]*ManagedProcess {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
...
func (pm *ProcessManager) GetProcessStats() map[string]ProcessStats {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
...
func (pm *ProcessManager) StartTypeScriptWatch(ctx *core.JawtContext) error {
	options := ProcessOptions{
		...
		OutputHandler: func(output string) {
			pm.eventBus.Publish(runtime.ProcessOutputEvent("process_manager", "typescript", output))
		},
		ErrorHandler: func(err error) {
			pm.eventBus.Publish(runtime.ProcessErrorEvent("process_manager", "typescript", err))
		},
	}
...
func (pm *ProcessManager) StartTailwindWatch(ctx *core.JawtContext) error {
	options := ProcessOptions{
		...
		MaxRestarts:      10,
		OutputHandler: func(output string) {
			pm.eventBus.Publish(runtime.ProcessOutputEvent("process_manager", "tailwind", output))
		},
		ErrorHandler: func(err error) {
			pm.eventBus.Publish(runtime.ProcessErrorEvent("process_manager", "tailwind", err))
		},
	}
...
func (pm *ProcessManager) StartDevServer(ctx *core.JawtContext, serverBinary string) error {
	options := ProcessOptions{
		...
		MaxRestarts:      5,
		OutputHandler: func(output string) {
			pm.eventBus.Publish(runtime.ProcessOutputEvent("process_manager", "devserver", output))
		},
		ErrorHandler: func(err error) {
			pm.eventBus.Publish(runtime.ProcessErrorEvent("process_manager", "devserver", err))
		},
	}
...
func (pm *ProcessManager) Health() map[string]bool {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
