...
func NewManagedProcess(name string, options ProcessOptions, ctx context.Context, logger core.Logger, eventBus core.EventBus) *ManagedProcess {
	...
	for {
		...
		default:
			if err := mp.runOnce(); err != nil {
				...
				mp.eventBus.Publish(runtime.ProcessErrorEvent("managed_process", mp.name, err))
...
func (mp *ManagedProcess) runOnce() error {
	...
	mp.eventBus.Publish(runtime.CreateProcessStopEvent("managed_process", mp.name, mp.pid))
